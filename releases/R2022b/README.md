...
| **Enable Nice Dcv** | Select 'Yes' if you wish to start desktop using [NICE DCV](https://www.nice-dcv.com). This will configure NICE DCV using a 30-days demo license (unless a production license is provided). You can revert to using xRDP for remote connection to this VM using Azure run-command mentioned in (README link here). By using NICE DCV, you agree to the terms and conditions outlined in [NICE DCV End User License Agreement](https://www.nice-dcv.com/eula.html). If you select 'No', NICE DCV will not be configured and you can connect to this VM using a RDP connection. |
